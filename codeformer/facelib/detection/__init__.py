...
model_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'weights', 'facelib')
config_dir = os.path.join(os.path.dirname(__file__), 'yolov5face', 'models')
...
def init_yolov5face_model(model_name, device='cuda'):
    if model_name == 'YOLOv5l':
        model = YoloDetector(config_name=os.path.join(config_dir, 'yolov5l.yaml'), device=device)
        model_url = 'https://github.com/sczhou/CodeFormer/releases/download/v0.1.0/yolov5l-face.pth'
    elif model_name == 'YOLOv5n':
        model = YoloDetector(config_name=os.path.join(config_dir, 'yolov5n.yaml'), device=device)
